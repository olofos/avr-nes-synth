...
void Nes_Apu::reset( bool pal_mode, int initial_dmc_dac )
{
	...
	frame_delay = 1;
	write_register( 0, 0, 0x4017, 0x00 );
	write_register( 0, 0, 0x4015, 0x00 );
	
	for ( nes_addr_t addr = start_addr; addr <= 0x4013; addr++ )
            write_register( 0, 0, addr, (addr & 3) ? 0x00 : 0x10 );

        RegWrite r = { 0, 0, 0 };
        reg_writes.push_back(r);
...
void Nes_Apu::write_register( nes_time_t time, long total_time, nes_addr_t addr, int data )
{
	...
	run_until_( time );
        ...
        RegWrite r = { total_time + time, (int) addr, data };
        reg_writes.push_back(r);
